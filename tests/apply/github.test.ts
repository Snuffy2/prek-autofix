...
describe("GitHub apply adapters", () => {
  it("maps lightweight workflow and fork PR reads through the base repository", async () => {
    ...
    await expect(
      read.listAssociatedPullRequests("head"),
  ...
  it("walks only affected paths through the base repository and caches shared ancestors", async () => {
    ...
    await expect(read.getCommitTreeSha("base/repo", "head")).resolves.toBe(
      "root",
    );
    await expect(
    ...
    expect(
      getTree.mock.calls.flatMap((call) => Object.keys(call[0])),
    ).not.toContain("recursive");
  });
  ...
  it("rejects over-budget paths before making a Git tree request", async () => {
    ...
    const read = createReadClient(octokit as never, "base", "repo");
    const pathCount = 100;
    const componentsPerPath = Math.floor(MAX_TOTAL_PATH_COMPONENTS / pathCount);
