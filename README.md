...
comment with a safe reason and links to the fix and source runs. Keep
...
| Cross-repository pull request without `PREK_AUTOFIX_TOKEN`                                        | Fix reports that it cannot update the fork and explains how to configure a token or run `prek run -a` locally.                         |
| Fork without maintainer edits                                                                     | Review still runs. Fix leaves one persistent PR comment with the reason, run links, and recovery steps.                                |
...
| Fork update denied                                    | Ask the contributor to enable **Allow edits from maintainers**. They can also run the repository's configured prek command locally and push the changes themselves.                                                                                                                             |
